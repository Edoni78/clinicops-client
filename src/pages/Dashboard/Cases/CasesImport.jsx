...
import {
  apiErrorMessage,
  confirmCaseMigration,
  listCaseMigrationRows,
...
export default function CasesImport() {
  const { role } = useAuth();
  ...
  const loadRows = useCallback(
    async (id, status, nextPage) => {
      const data = await listCaseMigrationRows(id, {
        status,
        page: nextPage,
        pageSize,
      });
  ...
  const handleUpload = async () => {
    ...
    setBusy(true);
    try {
      const data = await uploadCaseMigration(file);
      ...
      setResult(null);
      setStep(2);
      notify("success", "Kolonat e Excel-it u lexuan. Hartëzoni fushat e rastit.");
    } catch (err) {
  ...
  const handlePreview = async () => {
    ...
    setBusy(true);
    try {
      const data = await previewCaseMigration(migrationId, mappings);
      setPreview(data);
      setRowFilter("All");
      setPage(1);
  ...
  const handleConfirm = async () => {
    ...
    const ok = await confirm({
      title: "Konfirmo importin e rasteve",
      message: `${valid.toLocaleString("sq-AL")} raste janë gati për import si të mbyllura. ${duplicates.toLocaleString("sq-AL")} rreshta dublikatë do të anashkalohen. ${invalid.toLocaleString("sq-AL")} rreshta të pavlefshëm nuk do të importohen.`,
      confirmLabel: `Importo ${valid.toLocaleString("sq-AL")} raste`,
      cancelLabel: "Anulo",
      variant: "primary",
    });
    if (!ok) return;
    ...
    setStep(4);
    try {
      const data = await confirmCaseMigration(migrationId);
      setResult(data);
      setStep(5);
      notify("success", "Importi i rasteve përfundoi.");
    } catch (err) {
      setStep(3);
      notify("error", apiErrorMessage(err, "Importi dështoi. Asnjë rast nuk u shtua."));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="page-shell max-w-6xl">
        <PageHeader
          title="Importo raste"
          subtitle="Ngarkoni raste nga sistemi i vjetër. Çdo rresht lidhet me një pacient ekzistues. Të gjitha rastet importohen si të mbyllura."
          icon={FiUploadCloud}
          actions={
            <Link to="/dashboard/cases" className="btn-secondary btn-md">
              <FiFolder size={18} />
              Lista e rasteve
            </Link>
          }
        ...
        {step === 2 && (
          <div className="card-padded">
            ...
            <p className="text-sm text-slate-500 mt-1 mb-5">
              ...
              {" · "}
              {headers.length} kolona të gjetura. Data e lindjes është opsionale — lëreni bosh nëse Excel-i nuk e ka. Të gjitha rastet do të importohen si të mbyllura.
            </p>
        ...
        {(step === 3 || step === 4) && preview && (
          <div className="space-y-6">
            ...
            {step === 4 ? (
              <div className="card-padded">
                <LoadingSpinner className="py-10" label="Duke importuar rastet e vlefshme…" />
              </div>
            ) : (
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="table-head-row">
                          ...
                          <th className="table-th">Protokolli</th>
                          <th className="table-th">Statusi i rastit</th>
                          <th className="table-th">Rezultati</th>
                      ...
                      <tbody>
                        {rows.map((row) => (
                          <tr key={row.rowNumber ?? row.RowNumber} className="table-row">
                            ...
                            <td className="table-td">{row.protocolNumber ?? row.ProtocolNumber ?? "—"}</td>
                            <td className="table-td">{row.caseStatus ?? row.CaseStatus ?? "Mbyllur"}</td>
        ...
        {step === 5 && result && (
          <div className="space-y-6">
            <div className="card-padded text-center">
              ...
              <h2 className="text-xl font-semibold text-slate-900">Importi i rasteve përfundoi</h2>
              <p className="text-sm text-slate-500 mt-2 max-w-lg mx-auto">
                Rastet e vlefshme u shtuan si të mbyllura te pacientët e klinikës tuaj. Rreshtat dublikatë dhe të pavlefshëm nuk u importuan.
              </p>
            </div>
...
function ErrorRows({ migrationId }) {
  ...
  React.useEffect(() => {
    let active = true;
    listCaseMigrationRows(migrationId, { status: "Invalid", page: 1, pageSize: 100 })
      .then((data) => {
        if (!active) return;
